...
def main():
    ...
    # Step 5: Perform Similarity Checker (Plagiarism check)
    print("\nRunning plagiarism similarity scanner...")
    import difflib
    
    # Initialize similarity keys
    for r in results:
        r['similarity_flag'] = "N/A"
        r['similarity_percent'] = 0.0

    # Compare every success file with every other success file
    for i in range(len(results)):
        if not results[i]['success'] or not results[i]['source_code']:
            continue
        
        highest_match_percent = 0.0
        highest_match_student = ""
        
        code_i = [line.strip() for line in results[i]['source_code'].splitlines() if line.strip() and not line.strip().startswith('#')]
        flat_code_i = "\n".join(code_i)

        for j in range(len(results)):
            if i == j or not results[j]['success'] or not results[j]['source_code']:
                continue
            
            code_j = [line.strip() for line in results[j]['source_code'].splitlines() if line.strip() and not line.strip().startswith('#')]
            flat_code_j = "\n".join(code_j)

            # Compare contents
            matcher = difflib.SequenceMatcher(None, flat_code_i, flat_code_j)
            ratio = matcher.ratio() * 100
            
            if ratio > highest_match_percent:
                highest_match_percent = ratio
                highest_match_student = results[j]['student_name']

        if highest_match_percent >= 80.0:
            results[i]['similarity_flag'] = f"Flagged: {highest_match_percent:.1f}% match with {highest_match_student}"
            results[i]['similarity_percent'] = highest_match_percent
...
def generate_html_report(results: List[Dict[str, Any]]):
    html_lines = [
        ...
        "    .plag-flagged { color: #d63031; font-weight: bold; background-color: #ffeef0; padding: 4px 8px; border-radius: 4px; }",
        "    .plag-clear { color: #2f3542; font-style: italic; }",
        ...
        "</head>",
        "<body>",
        "  <h1>Google Classroom Auto-Compilation & Similarity Report</h1>",
        ...
        "  <table>",
        "    <thead>",
        "      <tr><th>Student Name</th><th>Student Email</th><th>File Name</th><th>Status</th><th>Similarity Flag (Plagiarism)</th><th>Notification Status</th><th>Error Description</th></tr>",
        "    </thead>",
    ...
    for r in results:
        ...
        # Color code similarity output
        sim_val = r['similarity_flag']
        if "Flagged" in sim_val:
            sim_class = "plag-flagged"
        else:
            sim_class = "plag-clear"
        ...
        html_lines.append(
            ...
            f"        <td><span class='{sim_class}'>{html.escape(sim_val)}</span></td>"
